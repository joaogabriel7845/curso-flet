...
class TodoApp(ft.Column):
    def __init__(self):
        ...
        self.items_left = ft.Text('0 Tarefa(s) não concluída(s)')
    ...
    def atualizar_tarefas(self):
        ...
        if task.completed:
            task.complete += 1
            if task.complete == len(self.tasks.controls):
                self.button_clear.disabled = False
            elif task.complete != len(self.tasks.controls):
                self.button_clear.disabled = False
            else:
                self.button_clear.disabled = False
        
        super().atualizar_tarefas()
